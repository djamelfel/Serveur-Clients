...
public class ClientV3{
	...
	ClientV3(int portS, int portC) throws IOException, SocketException, UnknownHostException{
		finProg=false;
		mot = "Bonjour";
		portClient=portC;			//nouvelle variable qui sera utilisee par la classe Ecoute
		msg = mot.getBytes();
		portServ=portS;
	...
	public void connexion() throws IOException {
		...
		if(mot.equals("Bonjour")){
			p.setData(nom.getBytes());
			p.setLength(nom.length());
			s.send(p);
			s.receive(p);
			portServ = p.getPort();
			ecoute();			//la connexion maintenant etablie nous faisons appel a la methode ecoute.
			envoiMes();
		}
	}
